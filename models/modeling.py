...
class Encoder(nn.Module):
    ...
    def forward(self, hidden_states, img, is_crop):
        ...
        ########## start crop ##########
        if is_crop:
            crop_imgs, alive_rate = attention_crop(all_attention_weights[10], img, self.heads_select_rate, self.total_num)
...
class Transformer(nn.Module):
    ...
    def forward(self, img, is_crop):
        ...
        part_encoded, crop_imgs = self.encoder(embedding_output, img, is_crop)
        return part_encoded, crop_imgs
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        tokens, crop_imgs = self.transformer(x, True)
        logits = self.part_head(tokens[:, 0])

        crop_tokens, _ = self.transformer(crop_imgs, False)
